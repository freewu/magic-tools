...
<blockquote><p>把一张图片缩放到<b>指定尺寸</b>: 可以按原图比例缩放 (25% / 50% / 200% …), 也可以直接指定目标宽高像素, 并锁定宽高比避免变形。适合上传前压缩尺寸、生成缩略图、把大图缩到某个平台要求的尺寸。</p></blockquote>
...
<li><p>按需开启「不放大图片」(仅缩小)、选择输出格式与 JPEG 质量, 右侧会实时给出结果预览与新尺寸</p></li>
...
<li><p>缩小超过一半时会<b>分多步逐步绘制</b>, 避免一次性大比例缩放导致的锯齿与模糊 (浏览器内置高质量插值)</p></li>
<li><p>「不放大图片」开启时, 结果不会超过原图尺寸: 超出时会等比缩回原图大小, 避免把截图放大后变糊</p></li>
<li><p>输出 PNG 无损且保留透明通道; 输出 JPEG 体积更小, 适合照片 (质量可调, 默认 0.92, 透明区域会自动铺白底)</p></li>
<li><p>设置里可配置默认缩放比例 / 默认输出格式 / 默认 JPEG 质量 / 默认锁定宽高比, 打开工具即生效</p></li>
</ul>

<h2>说明</h2>
<ul>
<li><p>缩放完全在本地浏览器 (或桌面端 WebView) 内完成, 图片不会上传到任何服务器</p></li>
<li><p>「结果体积」是按导出编码后的数据估算的, 用于对比压缩效果; 实际写入磁盘的字节数与之基本一致</p></li>
</ul>`;

const tw = `<h2>這個工具做什麼</h2>
<blockquote><p>把一張圖片縮放到<b>指定尺寸</b>: 可以依原圖比例縮放 (25% / 50% / 200% …), 也可以直接指定目標寬高像素, 並鎖定長寬比避免變形。適合上傳前壓縮尺寸、產生縮圖、把大圖縮到某個平台要求的尺寸。</p></blockquote>
...
<li><p>視需要開啟「不放大圖片」(僅縮小)、選擇輸出格式與 JPEG 品質, 右側會即時給出結果預覽與新尺寸</p></li>
...
<li><p>縮小超過一半時會<b>分多步逐步繪製</b>, 避免一次性大比例縮放導致的鋸齒與模糊 (瀏覽器內建高品質插值)</p></li>
<li><p>「不放大圖片」開啟時, 結果不會超過原圖尺寸: 超出時會等比縮回原圖大小, 避免把截圖放大後變糊</p></li>
<li><p>輸出 PNG 無損且保留透明通道; 輸出 JPEG 體積較小, 適合照片 (品質可調, 預設 0.92, 透明區域會自動鋪白底)</p></li>
<li><p>設定裡可配置預設縮放比例 / 預設輸出格式 / 預設 JPEG 品質 / 預設鎖定長寬比, 開啟工具即生效</p></li>
</ul>

<h2>說明</h2>
<ul>
<li><p>縮放完全在本機瀏覽器 (或桌面端 WebView) 內完成, 圖片不會上傳到任何伺服器</p></li>
<li><p>「結果體積」是依匯出編碼後的資料估算的, 用於對比壓縮效果; 實際寫入磁碟的位元組數與之基本一致</p></li>
</ul>`;

const en = `<h2>What this tool does</h2>
<blockquote><p>Resize an image to <b>a specific size</b>: scale it by a percentage of the original (25% / 50% / 200% …), or enter the exact target width and height while locking the aspect ratio so nothing gets stretched. Useful for shrinking images before upload, generating thumbnails, or hitting a platform's required dimensions.</p></blockquote>
...
<li><p>Optionally enable "Never enlarge" (shrink only) and choose the output format and JPEG quality — the result preview and the new dimensions update instantly</p></li>
...
<li><p>When reducing by more than half, the image is <b>drawn in several steps</b> to avoid the aliasing and blur of a single large downscale (using the browser's high-quality interpolation)</p></li>
...
<li><p>PNG output is lossless and keeps transparency; JPEG is smaller and better suited to photos (quality adjustable, 0.92 by default, transparent areas are filled with white)</p></li>
<li><p>Settings let you configure the default percentage, output format, JPEG quality and aspect-ratio lock, applied as soon as the tool opens</p></li>
</ul>

<h2>Notes</h2>
<ul>
<li><p>Resizing happens entirely in the local browser (or desktop WebView); the image is never uploaded anywhere</p></li>
