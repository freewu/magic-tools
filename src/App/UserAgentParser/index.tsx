      ...
      items={[
        {
          key: 'parse',
          label: (
            <span><CompassOutlined /> {t('解析')}</span>
          ),
          children: (
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <Alert
                ...
                showIcon
                message={t('UA 解析')}
                description={t('从 User-Agent 字符串中提取浏览器名称与版本、渲染引擎、操作系统、CPU 架构与设备信息。页面加载时已自动载入当前浏览器的 UA，也可粘贴任意 UA 字符串手动分析。')}
              />
              <Card size="small" title={t('User-Agent 字符串')} extra={
                <Text type="secondary" style={{ fontSize: 12 }}>{tt('{c} 字符', { c: ua.length })}</Text>
              }>
                <Input.TextArea
                  value={ua}
                ...
                <Card size="small" title={t('解析结果')}>
                  <Descriptions
                    ...
                    items={[
                      {
                        key: 'browser',
                        label: <Space size={4}><CompassOutlined /> {t('浏览器 / 程序')}</Space>,
                        children: (
                          <Space wrap size={6}>
                            <Text strong>{info.browserName || '—'}</Text>
                            {info.browserVersion && <Text code>{info.browserVersion}</Text>}
                            {isBot && info.botName && <Tag color="red">{t('爬虫 / 非浏览器')}</Tag>}
                          </Space>
                        ),
                      },
                      { key: 'engine', label: t('渲染引擎'), children: info.engine === '未知' ? t('未知') : info.engine },
                      {
                        key: 'os',
                        label: t('操作系统'),
                        children: info.osName !== '未知'
                          ? <Space size={6}><span>{info.osName === '未知' ? t('未知') : info.osName}</span>{info.osVersion && <Text code>{info.osVersion}</Text>}</Space>
                          : '—',
                      },
                      { key: 'cpu', label: t('CPU 架构'), children: info.cpuArch || '—' },
                      {
                        key: 'device',
                        label: t('设备类型'),
                        children: <Tag color={isBot ? 'red' : info.deviceType === 'desktop' ? 'blue' : 'green'}>{t(DEVICE_LABEL[info.deviceType])}</Tag>,
                      },
                      { key: 'model', label: t('设备型号'), children: info.deviceModel || '—' },
                    ]}
                  />
                  {isBot && (
                    <Alert
                      ...
                      type="warning"
                      showIcon
                      message={tt('检测到非浏览器程序: {name}', { name: info.botName ?? '' })}
                      description={t('该 UA 通常来自搜索引擎爬虫或命令行/脚本请求，不是真实用户的浏览器。')}
                    />
                  )}
                </Card>
              ) : (
                <Alert type="warning" showIcon message={t('请输入 User-Agent 字符串进行分析')} />
              )}

              <Card size="small" title={t('常见 UA 速查')} extra={<Button size="small" type="link" onClick={() => setUa('')}>{t('清空')}</Button>}>
                <Space wrap size={8}>
                  {COMMON_UAS.map(([label, u]) => (
                    <Button key={label} size="small" onClick={() => { setUa(u); message.info(tt('已载入 {label} UA', { label })); }}>{label}</Button>
                  ))}
                </Space>
              </Card>
            </Space>
        ...
        },
        {
          key: 'generate',
          label: (
            <span><ExperimentOutlined /> {t('生成')}</span>
          ),
          children: (
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <Alert
                ...
                description={
                  <>
                    {t('勾选需要的「平台」与「浏览器」（可多选），点')}
                    <Text strong>{t('生成 UA 列表')}</Text>{t(' 即可得到全部组合的 UA 字符串，每条可单独复制或载入「解析」tab 验证。')}
                    {t('Safari 仅有 macOS / iOS 版本，不支持的组合会自动跳过并标注。')}
                  </>
                }
              />
              <Card size="small" title={t('组合条件')} extra={
                ...
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  <div>
                    <Text type="secondary" style={{ fontSize: 12, marginRight: 8 }}>{t('平台')}</Text>
                    <Checkbox.Group
                  ...
                  <div>
                    <Text type="secondary" style={{ fontSize: 12, marginRight: 8 }}>{t('浏览器')}</Text>
                    <Checkbox.Group
                  ...
                  <Space size={8}>
                    <Text type="secondary" style={{ fontSize: 12 }}>{t('浏览器版本')}</Text>
                    <Input
                      size="small"
                      style={{ width: 180 }}
                      value={genVer}
                      onChange={(e) => setGenVer(e.target.value.replace(/[^\d.]/g, ''))}
                      placeholder={t('留空 = 各浏览器默认较新版本')}
                      allowClear
                    />
                    <Text type="secondary" style={{ fontSize: 12 }}>{t('（可选，例如 130）')}</Text>
                  </Space>
                </Space>
              </Card>

              {generated ? (
                <Card size="small" title={tt('生成结果 ({n} 条可用)', { n: generated.filter((g) => g.ua).length })} extra={
                  ...
                  <List
                    ...
                    renderItem={(g) => (
                      <List.Item
                        ...
                        actions={g.ua ? [
                          <Button key="c" size="small" icon={<CopyOutlined />} onClick={() => copy(g.ua!)}>{t('复制')}</Button>,
                          <Button key="p" size="small" onClick={() => { setUa(g.ua!); message.success(tt('已载入「{label}」到解析', { label: g.label })); }}>
                            {t('载入解析')}
                          </Button>,
                        ] : []}
                      >
                        <Space direction="vertical" size={4} style={{ width: '100%', paddingRight: 8 }}>
                          <Space size={6}>
                            ...
                            {!g.ua && <Text type="secondary" style={{ fontSize: 12 }}>{t('该浏览器无此平台版本')}</Text>}
                          </Space>
                          {g.ua && (
                ...
                <Alert type="info" showIcon message={t('尚未生成 — 勾选上方平台与浏览器组合后点击「生成 UA 列表」。')} />
              )}
            </Space>
          ),
        },
