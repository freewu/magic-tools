...
const imagelangRows: Record<string, [string, string]> = {
  ...
  'Shiki 语法高亮 + 一键导出 PNG。支持全部内置语言（常用语言置顶），编辑器风格可选 Mac / VSCode / IntelliJ / Sublime / Vim / Emacs，明暗自适应配色，默认值均可在「设置 → 图片」中调整。': ['Shiki 語法高亮 + 一鍵匯出 PNG。支援全部內建語言（常用語言置頂），編輯器風格可選 Mac / VSCode / IntelliJ / Sublime / Vim / Emacs，明暗自適應配色，預設值均可在「設定 → 圖片」中調整。', 'Shiki syntax highlighting with one-click PNG export. All built-in languages are supported (common ones on top); editor styles include Mac / VSCode / IntelliJ / Sublime / Vim / Emacs with light/dark-adaptive themes. Defaults can be changed under Settings → Image.'],
  ...
  '导出尺寸 = 截图内容实际像素 × 2 (pixelRatio); 行号随行对齐并随 PNG 一并导出。最小宽度为 0 时截图宽度按代码长度自适应。若内边距较小时圆角会自动收小, 避免代码压弧产生暗圈。': ['匯出尺寸 = 截圖內容實際像素 × 2 (pixelRatio); 行號隨行對齊並隨 PNG 一併匯出。最小寬度為 0 時截圖寬度依程式碼長度自適應。若內邊距較小時圓角會自動縮小, 避免程式碼壓弧產生暗圈。', 'Export size = the actual pixels of the captured content × 2 (pixelRatio); line numbers stay aligned with lines and are exported with the PNG. With a min width of 0 the shot width fits the code. When padding is small the corner radius shrinks so the code never touches the arc edge.'],
