...
const PlaceholderImage: React.FC = () => {
  ...
  const presetOptions = [
    ...
    ...(customs.length > 0
      ? [ { label: t('我的预设'), options: customs.map((d) => ({ value: dimKey(d.w, d.h), label: `${d.w} × ${d.h}` })) } ]
      : []),
  ];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (id === genIdRef.current) message.error(err instanceof Error ? t(err.message) : t('生成失败'));
      }
    })();
  ...
  const save = async () => {
    ...
    try {
      ...
      const ok = await saveBytesFile(name, result.bytes, {
        title: tt('保存占位图 {name}', { name }),
        filterName: t(FILTER_NAME[result.fmt]),
        extensions: [ result.fmt ],
      });
      if (ok) {
        message.success(tt('已保存 {name}', { name }));
        setModalOpen(false);
      }
    } finally {
      setSaving(false);
  ...
  return (
    <div>
      <div style={ { maxWidth: 720 } }>
        {/* 参数行: 预设 + 宽高 */}
        <div style={ row }>
          <span style={ label }>{t('预设')}</span>
          <Select
            value={ presetKey }
            placeholder={t('选常用尺寸填充')}
          ...
          />
          <span style={ label }>{t('尺寸')}</span>
          <InputNumber min={ PH_MIN } max={ PH_MAX } value={ w } onChange={ (v) => { if (v != null) { setPresetKey(null); setW(v); } } } addonBefore={t('宽')} style={ { width: 130 } } />
          <span style={ { color: '#bbb' } }>×</span>
          <InputNumber min={ PH_MIN } max={ PH_MAX } value={ h } onChange={ (v) => { if (v != null) { setPresetKey(null); setH(v); } } } addonBefore={t('高')} style={ { width: 130 } } />
          <span style={ label }>{t('格式')}</span>
          <Select value={ fmt } onChange={ setFmt } style={ { width: 90 } }
            options={ PH_FORMATS.map((f) => ({ value: f, label: f.toUpperCase() })) } />
        </div>
        {/* 文字 */}
        <div style={ row }>
          <span style={ label }>{t('显示文字')}</span>
          <Input value={ text } onChange={ (e) => setText(e.target.value) } style={ { width: 320, maxWidth: '100%' } }
            placeholder={ tt('留空则显示默认文字: {d}', { d: defaultText }) } allowClear />
          { !text && <span style={ { color: '#bbb', fontSize: 12 } }>{t('默认')} { defaultText }</span> }
        </div>
        {/* 颜色 */}
        <div style={ row }>
          <span style={ label }>{t('背景颜色')}</span>
          ...
          <Button type="primary" onClick={ generate }>{t('生成图片')}</Button>
        </div>
        <div style={ { color: '#bbb', fontSize: 12, marginBottom: 10 } }>
          { tt('宽高范围 {a}–{b}px; 文字过长会自动缩小字号或截断; 已生成后修改参数会自动刷新图片; 常用尺寸见预设下拉, 设置页可自定预设', { a: PH_MIN, b: PH_MAX }) }
        </div>
      </div>

      <Divider style={ { margin: '8px 0' } }>{t('预览 (点击图片弹窗保存)')}</Divider>
      ...
      { result ? (
        <div style={ { display: 'inline-block' } }>
          ...
          <div style={ { color: '#aaa', marginTop: 6, fontSize: 12 } }>
            { tt('{w}×{h} · {f} · 点击图片查看并保存', { w: result.w, h: result.h, f: result.fmt.toUpperCase() }) }
          </div>
        </div>
      ) : (
        <div style={ { color: '#ccc', padding: '20px 0' } }>
          {t('设置参数后点击「生成图片」, 图片将显示在此处; 点击图片可弹窗下载保存')}
        </div>
      ) }
      ...
      <Modal
        open={ modalOpen }
        title={ result ? tt('保存占位图 · {s}', { s: result.w + '×' + result.h + ' ' + result.fmt.toUpperCase() }) : '' }
        onCancel={ () => setModalOpen(false) }
        width={ 480 }
        footer={ [
          <Button key="cancel" onClick={ () => setModalOpen(false) }>{t('取消')}</Button>,
          <Button key="save" type="primary" loading={ saving } onClick={ save }>{t('保存图片')}</Button>,
        ] }
      >
        <div style={ { textAlign: 'center', padding: '8px 0' } }>
          ...
          <div style={ { color: '#888', marginTop: 10, fontSize: 12 } }>
            {t('将保存为')} <b style={ { fontFamily: 'monospace' } }>{ fileName }</b>{ tt(' ({kb} KB)', { kb: result ? Math.round(result.bytes.length / 1024) : 0 }) }
          </div>
        </div>
      </Modal>
    </div>
