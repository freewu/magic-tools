...
const BcryptCalc = () => {
  ...
  const check = async () => {
    ...
    try {
      ...
      notice.error(tt('校验失败: {m}', { m: (err as Error).message }));
      setMatch(null);
    } finally {
      setChecking(false);
    }
  ...
  return (
    <div style={ { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } }>
      ...
      <Alert
        type="info"
        showIcon
        message={t('BCrypt 内置随机盐, 相同口令每次生成结果不同; 使用前请确认与目标系统相同的 $2a/$2b/$2y 前缀与成本因子')}
      />
      <Divider orientation="left" plain>{t('生成哈希')}</Divider>
      <Space direction="vertical" style={ { width: '100%' } } size={ 4 }>
        <Form.Item label={t('口令')} style={ { marginBottom: 8 } }>
          <Input
            allowClear
            value={ password }
            placeholder={t('输入需要计算 BCrypt 哈希的口令 (超过 72 字节的内容会被截断)')}
        ...
        <Form.Item label="BCrypt" style={ { marginTop: 8, marginBottom: 0 } }>
          <Input readOnly showCount value={ hash } placeholder={t('生成结果, 点击可复制')} onClick={ inputClick } />
        </Form.Item>
      </Space>

      <Divider orientation="left" plain>{t('校验')}</Divider>
      <Form layout="inline" style={ { flexWrap: 'wrap', rowGap: 8 } }>
        <Form.Item label={t('口令')}>
          <Input
            allowClear
      ...
      { match !== null && (
        <Tag color={ match ? 'green' : 'red' } style={ { marginTop: 8, width: 'fit-content' } }>
          { match ? t('校验通过: 口令与哈希匹配') : t('校验失败: 口令与哈希不匹配') }
        </Tag>
      ) }
      { verifyHash.trim() !== '' && isValidBcryptHash(verifyHash.trim()) && bcryptCostOf(verifyHash.trim()) !== cost && (
        <div style={ { marginTop: 4, color: '#999', fontSize: 12 } }>
          {tt('提示: 目标哈希成本因子为 {a}, 与上方生成区当前成本 {b} 不同', { a: String(bcryptCostOf(verifyHash.trim())), b: cost })}
        </div>
      ) }
    </div>
  );
