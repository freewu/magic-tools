...
const nonceHint = ' 24 HEX 或 12 字符';
...
const ChaCha20Crypto = () => {
  ...
  const encode = () => {
    ...
    if(passphrase.trim() === '') { notice.warning('密钥口令不能为空'); return; }
    if('' !== nonceStatus) { notice.warning('nonce 格式不正确'); return; }
    if('' !== counterStatus) { notice.warning('计数器需为 0~2^32-1 的整数'); return; }
    try {
  ...
  return (
    <div>
      ...
      <Row style = { { marginTop: "5px" }}>
        <Space>
          ...
          <Input
            ...
            value= { passphrase } />
          <span style={{ color: '#999' }}>口令经 SHA-256 派生为 32 字节密钥 (恰 32 字符时直接使用)</span>
        </Space>
      </Row>
      <Row style = { { marginTop: "5px" }}>
        <Space>
          ...
          <Input
            ...
            value= { nonce } />
          <span>{ nonce.length } / { nonceHint }</span>
          <label>计数器(初始值):</label>
          <InputNumber
            min={ 0 }
            max={ COUNTER_MAX }
      ...
      <TextArea
        ...
        title="双击复制内容到粘贴板"
        value= { encodeValue }
        placeholder="输入需要进行 ChaCha20 加密的内容  或 拖拽文件到框内打开"
      ...
      <TextArea
        ...
        title="双击复制内容到粘贴板"
        value= { decodeValue }
        placeholder="输入需要进行 ChaCha20 解密的内容  或 拖拽文件到框内打开"
        autoSize={{ minRows: 8, maxRows: 8 }}
        onDragOver={ (e) => { e.preventDefault(); } } // 必须加上，否则无法触发下面的方法
