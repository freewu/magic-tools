...
<h2>使用步骤</h2>
<ul>
<li><p><b>JSON → JSONL</b>: 顶部粘贴 JSON 数组 (或单个 JSON 对象 / 值), 点击蓝色按钮, 每个数组元素会被压成一行紧凑 JSON</p></li>
...
<li><p>整段内容本身就是一个 JSON 数组时, 在 JSONL 方向会提示改用「JSON → JSONL」, 避免把外层数组当成一行数据</p></li>
...
<h2>使用步驟</h2>
<ul>
<li><p><b>JSON → JSONL</b>: 上方貼上 JSON 陣列 (或單一 JSON 物件 / 值), 點擊藍色按鈕, 每個陣列元素會被壓成一列緊湊 JSON</p></li>
...
<li><p>整段內容本身就是一個 JSON 陣列時, 在 JSONL 方向會提示改用「JSON → JSONL」, 避免把外層陣列當成一筆資料</p></li>
...
<h2>How to use it</h2>
<ul>
<li><p><b>JSON → JSONL</b>: paste a JSON array (or a single JSON object/value) on top and click the blue button — every element becomes one compact JSON line</p></li>
...
<li><p>If the whole input is itself a JSON array, the JSONL direction tells you to use "JSON → JSONL" so the wrapping array is not mistaken for one record</p></li>
