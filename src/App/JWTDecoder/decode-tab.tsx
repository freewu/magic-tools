...
/** 「解析」页签: 粘贴 JWT 自动解码头部 / 负载 / 签名 */
const JWTDecodeTab = () => {

  const { locale } = useLocale();
  const t = (key: string, fallback: string) => tr(jwtLang, locale, key, fallback);
  const tpl = (key: string, vars: Record<string, string | number>, fallback: string) => trTpl(jwtLang, locale, key, vars, fallback);
  ...
  return (
    <div>
      ...
      <TextArea
        ...
        value={ token }
        placeholder={ t('placeholder', '粘贴 JWT (header.payload.signature, 例如 eyJhbGciOi...)') }
      ...
      { result.ok && result.header && result.payload && (
        <div style={ { margin: "8px 0" } }>
          <Typography.Text type="secondary" style={ { fontSize: 12 } }>
            { tpl('metaInfo', { meta: headerMeta || t('metaNone', '无 alg/typ/kid 字段') }, '') }
          </Typography.Text>

          <div style={ { marginTop: 12, fontWeight: 600 } }>{ t('headerTitle', '① 头部 (Header)') }</div>
          <TextArea
            readOnly
          ...
          <div style={ { fontWeight: 600 } }>{ t('payloadTitle', '② 负载 (Payload)') }</div>
          <TextArea
            readOnly
            style={ { margin: "4px 0 8px 0", background: "transparent" } }
            onDoubleClick={ textareaDoubleClick }
          ...
          <div style={ { fontWeight: 600, marginBottom: 4 } }>{ t('sigTitle', '③ 签名 (Signature)') }</div>
          <Input
            readOnly
            value={ sig === '' ? t('sigEmpty', '(空签名, 可能为 none 算法)') : sig.toUpperCase() }
            onClick={ inputClick }
            title={ t('clickTitle', '点击复制 HEX 签名') }
            addonBefore="HEX"
      ...
      <Divider>{ t('divider', 'JWT 说明') }</Divider>
