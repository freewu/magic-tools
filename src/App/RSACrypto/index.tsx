...
const RSACrypto = () => {
  ...
  const generate = async () => {
    if (!rsaAvailable()) {
      notice.error(t('当前环境不支持 WebCrypto, 无法生成密钥'));
      return;
    }
    setLoading(true);
    try {
      ...
      notice.success(tt('RSA-{bits} 密钥对生成成功, 已保存为默认密钥', { bits: keyBits }));
    } catch (err) {
      notice.error(tt('生成失败: {m}', { m: crErr(locale, (err as Error).message) }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportPem = async (which :'public' | 'private') => {
    ...
    if (pem.trim() === '') { notice.warning(t('没有可导出的密钥, 请先生成')); return; }
    const ok = await saveTextFile(`rsa-${keyBits}-${which}.pem`, pem, t('保存密钥文件'));
  ...
  const doEncrypt = async () => {
    ...
    if (!rsaAvailable()) { notice.error(t('当前环境不支持 WebCrypto')); return; }
    setLoading(true);
    try {
  ...
  const doDecrypt = async () => {
    ...
    if (cipherValue.trim() === '') { notice.warning(t('请输入需要解密的密文 (Base64)')); return; }
    if (!rsaAvailable()) { notice.error(t('当前环境不支持 WebCrypto')); return; }
    setLoading(true);
    try {
  ...
  const manageTab = (
    <div>
      <Space wrap style={ { margin: "8px 0" } }>
        ...
        />
        <Button type="primary" icon={ <KeyOutlined /> } onClick={ generate } loading={ loading }>{t('生成密钥对')}</Button>
      ...
      <div style={ { fontWeight: 600 } }>{t('公钥 (SPKI PEM)')} <span style={ { color: "#999", fontWeight: 400 } }>{t('用于加密')}</span></div>
      <TextArea
        style={ { margin: "4px 0", fontFamily: "monospace", fontSize: 12 } }
        onChange={ (e) => setPublicPem(e.target.value) }
        onDoubleClick={ textareaDoubleClick }
        value={ publicPem }
        placeholder={t('-----BEGIN PUBLIC KEY----- ... (生成后自动填充, 也可粘贴其他工具导出的公钥)')}
        autoSize={{ minRows: 4, maxRows: 8 }}
      />
      <Space style={ { margin: "4px 0 8px 0" } }>
        <Button size="small" onClick={ () => copy(publicPem, '公钥') }>{t('复制公钥')}</Button>
        <Button size="small" onClick={ () => exportPem('public') }>{t('导出公钥到文件')}</Button>
      </Space>

      <div style={ { fontWeight: 600 } }>{t('私钥 (PKCS#8 PEM)')} <span style={ { color: "#999", fontWeight: 400 } }>{t('用于解密, 请保密')}</span></div>
      <TextArea
        style={ { margin: "4px 0", fontFamily: "monospace", fontSize: 12 } }
        onChange={ (e) => setPrivatePem(e.target.value) }
        onDoubleClick={ textareaDoubleClick }
        value={ privatePem }
        placeholder={t('-----BEGIN PRIVATE KEY----- ... (生成后自动填充, 也可粘贴其他工具导出的私钥)')}
        autoSize={{ minRows: 4, maxRows: 8 }}
      />
      <Space style={ { margin: "4px 0 8px 0" } }>
        <Button size="small" onClick={ () => copy(privatePem, '私钥') }>{t('复制私钥')}</Button>
        <Button size="small" onClick={ () => exportPem('private') }>{t('导出私钥到文件')}</Button>
      </Space>
    </div>
  );
  ...
  const cryptoTab = (
    <div>
      <div style={ { margin: "8px 0", background: "rgba(128,128,128,0.08)", padding: "8px 12px", borderRadius: 6 } }>
        <div>{t('当前公钥: ')}{ publicPem.trim() === '' ? <span style={ { color: "#dc3545" } }>{t('未配置')}</span> : <span style={ { fontFamily: "monospace", fontSize: 12 } }>{ pemSummary(publicPem) }</span> }</div>
        <div style={ { marginTop: 4 } }>{t('当前私钥: ')}{ privatePem.trim() === '' ? <span style={ { color: "#dc3545" } }>{t('未配置')}</span> : <span style={ { fontFamily: "monospace", fontSize: 12 } }>{ pemSummary(privatePem) }…</span> } <span style={ { color: "#999" } }>{t('(摘要)')}</span></div>
      </div>
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value={ plainValue }
        placeholder={t('输入需要加密的明文 (UTF-8, 超过 190 字节会自动分段)  或 拖拽文件到框内打开')}
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value={ cipherValue }
        placeholder={t('密文 (Base64, 每段 = 密钥模长) 加密后自动显示在此; 也可粘贴外部工具的密文后点「解密」  或 拖拽文件到框内打开')}
