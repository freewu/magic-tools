...
import { getDefaultCode, getDefaultPassphrase, getDefaultRound, xteaEncrypt, xteaDecrypt } from "./lib";
...
const XTEACrypto = () => {
  ...
  const encode = () => {
    ...
    try {
      const cipher = xteaEncrypt(utf8ToBytes(encodeValue), utf8ToBytes(passphrase), parseInt(round));
      switch(code) {
  ...
  const decode = () => {
    ...
    try {
      ...
      // 解密处理
      const result = bytesToUtf8(xteaDecrypt(cipher, utf8ToBytes(passphrase), parseInt(round)));
  ...
  return (
    <div>
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value= { encodeValue }
        placeholder={t('输入需要进行 XTEA 加密的内容 或 拖拽文件到框内打开')}
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value= { decodeValue }
        placeholder={t('输入需要进行 XTEA 解密的内容 或 拖拽文件到框内打开')}
        autoSize={{ minRows: 8, maxRows: 8 }}
        onDragOver={ (e) => { e.preventDefault(); } } // 必须加上，否则无法触发下面的方法
