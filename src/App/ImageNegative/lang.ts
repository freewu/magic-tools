// ImageNegative 语言包: 名称 (zh-CN = define.tsx AppName 默认; 缺省回退 zh-CN)
export default {
  default: 'zh-CN',
  'zh-TW': { appName: '圖片負片' },
  en: { appName: 'Image Negative' },
} as const;
...
const rows: Record<string, [string, string]> = {
  ...
  '负片强度': ['負片強度', 'Invert strength'],
  '{n}%': ['{n}%', '{n}%'],
  '0% 保留原图, 100% 完全反相 (黑变白 / 白变黑); 中间值相当于按该比例把负片叠在原图上': ['0% 保留原圖, 100% 完全反相 (黑變白 / 白變黑); 中間值相當於按該比例把負片疊在原圖上', '0% keeps the original, 100% fully inverts (black ↔ white); in between, the negative is blended over the original'],
  ...
  '只有 RGBA 颜色通道会被反相, α (透明度) 通道保持不变': ['只有 RGBA 顏色通道會被反相, α (透明度) 通道保持不變', 'Only the RGB channels are inverted; the alpha (transparency) channel is kept as is'],

  // 说明
  '图片负片说明': ['圖片負片說明', 'About image negative'],
};

// 取词: 无命中回退 zh 原文
export const nv = (locale: string, zh: string): string => {
  const e = rows[zh];
  if (!e) return zh;
  return locale === 'zh-TW' ? e[0] : locale === 'en' ? e[1] : zh;
};
export const nvT = (locale: string, zh: string, v?: Record<string, string | number>): string => {
  let s = nv(locale, zh);
