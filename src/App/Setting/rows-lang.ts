...
const ROWS: Record<string, Row> = {
  ...
  '打开页面时的初始内容, 默认值 ${d}, 留空则回退该值': ['開啟頁面時的初始內容, 預設值 ${d}, 留空則回退該值', 'Initial content when the page opens — default ${d}; blank falls back to it'],
  ...
  'SM9 页「解密」使用; 留空表示不配置': ['SM9 頁「解密」使用; 留空表示不設定', 'Used by the decrypt tab of the SM9 tool; leave empty to skip'],
  '默认签名主私钥 (DER HEX)': ['預設簽章主私鑰 (DER HEX)', 'Default SM9-Sign master private key (DER HEX)'],
  '用于导出签名主公钥; 留空表示不配置': ['用於匯出簽章主公鑰; 留空表示不設定', 'Used to export the signature master public key; leave empty to skip'],
  '默认签名用户私钥 (DER HEX)': ['預設簽章使用者私鑰 (DER HEX)', 'Default SM9-Sign user private key (DER HEX)'],
  'SM9 页「签名验签」使用; 留空表示不配置': ['SM9 頁「簽章驗簽」使用; 留空表示不設定', 'Used by the sign/verify tab of the SM9 tool; leave empty to skip'],
  '默认密钥保存在浏览器本地, 打开 SM9 页会自动带出': ['預設金鑰儲存在瀏覽器本機, 開啟 SM9 頁會自動帶出', 'Default keys are stored locally in the browser and auto-filled when opening the SM9 tool'],
