...
const SvgFormat: React.FC = () => {
  ...
  const copy = async () => {
    if (!result) return;
    try { await navigator.clipboard.writeText(result.data); message.success('已复制'); } catch { /* ignore */ }
  };
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Alert
        ...
        showIcon
        message="SVG 格式化 / 压缩"
        description="基于 SVGO 引擎处理 SVG: 去除注释、冗余属性、多余分组并优化路径等。可「美化」为多行缩进或「压缩」为单行最小体积, 适合发布到网页前瘦身。"
      />
      <Card size="small" title="SVG 源码" extra={
        <Space size={8}>
          <input
            ref={fileRef}
            type="file"
          ...
          />
          <Button size="small" icon={<UploadOutlined />} onClick={() => fileRef.current?.click()}>读取 .svg 文件</Button>
          <Button size="small" onClick={() => setRaw(SAMPLE_SVG)}>载入示例</Button>
      ...
      <Card size="small" title="优化选项与结果" extra={
        <Radio.Group
          ...
          buttonStyle="solid"
          options={[
            { label: '压缩 (单行)', value: 'min' },
            { label: '美化 (多行)', value: 'pretty' },
          ]}
        />
      }>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 24 }}><Spin tip="优化中…"><FileImageOutlined style={{ fontSize: 32 }} /></Spin></div>
        ) : err ? (
          <Alert type="error" showIcon message="处理失败" description={err} />
        ) : result ? (
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            <Space wrap size={8}>
              ...
              {saved > 0 && <Tag color="green">节省 {saved.toLocaleString()} B ({savedPct}%)</Tag>}
              {saved <= 0 && <Tag>体积无变化</Tag>}
              <Button size="small" icon={<CopyOutlined />} onClick={copy}>复制</Button>
              <Button size="small" icon={<DownloadOutlined />} onClick={download}>下载 .svg</Button>
            </Space>
            <Input.TextArea
              value={result.data}
              readOnly
            ...
            />
            <div>
              <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 6 }}>预览:</Text>
              <div style={{ border: '1px dashed #d9d9d9', borderRadius: 8, padding: 12, display: 'inline-block', background: '#fff' }}>
                <img src={previewUrl} alt="SVG 预览" style={{ maxWidth: 220, maxHeight: 220 }} />
              </div>
            </div>
          </Space>
        ) : (
          <Text type="secondary">输入 SVG 后自动优化。文件 → 网络: 平均可缩小 30%~60%。</Text>
        )}
      </Card>
    </Space>
  );
