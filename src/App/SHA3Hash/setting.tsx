...
export const SHA3HashSetting = () => {
  ...
  return (
    <>
      {contextHolder}
      <Divider orientation="left" plain>{ st('SHA3 Hash 值计算') }</Divider>
      <Form.Item label={ st('默认 SHAKE 输出长度') }>
        <InputNumber
          min={ 8 }
          max={ 8192 }
          step={ 8 }
          style={ { width: 140 } }
          title={ st('打开页面时 SHAKE128/256 的输出长度 (bit), 须为 8 的整数倍') }
          value={ shakeBits }
