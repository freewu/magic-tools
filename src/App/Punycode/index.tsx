...
import punycodeLang from "./lang";

const Punycode = () => {

  const { locale } = useLocale();
  const t = (key: string, fallback: string) => tr(punycodeLang, locale, key, fallback);
  ...
  const encode = () => {
    ...
    try {
      ...
      notice.error(t('encodeFailed', '编码失败！！！'));
    }
  }
  ...
  const decode = () => {
    ...
    try {
      ...
      setSourceStatus('error');
      notice.error(t('decodeFailed', '解码失败: 输入不是有效的 Punycode 文本！！！'));
    }
  }
  ...
  return (
    <div>
      ...
      <TextArea
        ...
        value= { sourceValue }
        placeholder={ t('encodePh', '输入需要编码为 Punycode 的文本 (如: 中文、中文.中国、bücher.de)  或 拖拽文件到框内打开') }
      ...
      <TextArea
        ...
        title={ t('copyTitle', '双击复制内容到粘贴板') }
        value= { targetValue }
        placeholder={ t('decodePh', '输入需要解码的 Punycode 文本 (如: xn--fiq228c、xn--fiq228c.xn--fiqs8s、xn--bcher-kva.de)  或 拖拽文件到框内打开') }
        autoSize={{ minRows: 5, maxRows: 5 }}
        onDragOver={ (e) => { e.preventDefault(); } }
        onDrop={ (e) => { e.preventDefault(); openFile(e.dataTransfer.files, setTargetValue ); } }
      />
      
      <Divider>{ t('divider', 'Punycode 编码说明') }</Divider>
