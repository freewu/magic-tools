...
import { rc2Encrypt, rc2Decrypt, rc2KeyValid, rc2IvValid, genCapacity } from "./lib";
import { getDefaultCode, getDefaultMode, getDefaultPadding, getDefaultIV, getDefaultPassphrase } from "./lib";
import type { InputStatus } from "antd/es/_util/statusUtils";


// RC2 加解密 (RC2-32/12/16, 分组 64 位, 密钥 128/192/256 位; 模式/填充/编码参考 AES)
const RC2Crypto = () => {
  ...
  const [ ivStatus, setIVStatus ] = useState(rc2IvValid(getDefaultIV()) ? '' : 'error' as InputStatus);
  const [ passphraseStatus, setPassphraseStatus ] = useState((rc2KeyValid(getDefaultPassphrase(), genCapacity(getDefaultPassphrase().length))) ? '' : 'error' as InputStatus);
  ...
  const encode = () => {
    ...
    try {
      setDecodeValue(rc2Encrypt(encodeValue, passphrase, {
  ...
  const decode = () => {
    ...
    try {
      setEncodeValue(rc2Decrypt(decodeValue, passphrase, {
        mode: mode as 'CBC' | 'CFB' | 'CTR' | 'OFB' | 'ECB',
        padding: padding as 'Pkcs7' | 'AnsiX923' | 'Iso10126' | 'Iso97971' | 'ZeroPadding',
        code: code as 'HEX' | 'Base64',
        capacity,
  ...
  const onIVChange = (e :React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIV(v);
    setIVStatus(rc2IvValid(v) ? '' : 'error');
  }

  // 位数切换 (128/192/256)
  const onCapacityChange = (n :number) => {
    setCapacity(n);
    setPassphraseStatus(rc2KeyValid(passphrase, n) ? '' : 'error');
  }
  ...
  const onModeChange = (v :string) => {
    setMode(v);
    setIVDisabled(v === 'ECB');
    setIVStatus((v === 'ECB' || rc2IvValid(iv)) ? '' : 'error');
  }
  ...
  const onPassphraseChange = (e :React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPassphrase(v);
    setPassphraseStatus(rc2KeyValid(v, capacity) ? '' : 'error');
  }
  ...
  return (
    <div>
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value= { encodeValue }
        placeholder={t('输入需要进行 RC2 加密的内容  或 拖拽文件到框内打开')}
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value= { decodeValue }
        placeholder={t('输入需要进行 RC2 解密的内容  或 拖拽文件到框内打开')}
        autoSize={{ minRows: 8, maxRows: 8 }}
        onDragOver={ (e) => { e.preventDefault(); } } // 必须加上，否则无法触发下面的方法
