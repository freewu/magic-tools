...
<li><p>「<b>按行填充</b>」开关打开时改为<b>一行练一个字</b>: 同一行重复同一个字, 第 N 行用第 N 个字 (关掉则逐格连续排字)</p></li>
...
<li><p>「<b>依行填充</b>」開關打開時改為<b>一行練一個字</b>: 同一行重複同一個字, 第 N 行用第 N 個字 (關掉則逐格連續排字)</p></li>
...
<li><p>Turn on "<b>One char per row</b>" to practise one character per row: every cell in a row repeats the same character, row N uses character N (off = characters flow cell by cell)</p></li>
