...
const RailFenceCrypto = () => {
  ...
  const doEncrypt = () => {
    if (rails === null || !Number.isInteger(rails) || rails < 2) { notice.warning(t('请输入栏数 (不小于 2 的整数)')); return; }
    ...
    <div>
      ...
      <Space wrap style={ { margin: "8px 0" } }>
        ...
        <InputNumber
          ...
          style={ { width: 140 } }
        />
        <span style={ { color: "#999" } }>{t('按锯齿形写入 N 栏后逐栏读出, 支持中文等任意字符 (2 栏时即奇偶位分离)')}</span>
      </Space>
      ...
      <TextArea
        ...
        title={t('双击复制内容到粘贴板')}
        value={ plainValue }
        placeholder={t('输入需要加密的明文, 例如 WEAREDISCOVEREDFLEEATONCE  或 拖拽文件到框内打开')}
