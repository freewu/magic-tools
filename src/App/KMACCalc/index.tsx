...
const KMACCalc = () => {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 8 }}><b>密钥 K (SP 800-185 建议 ≥ 目标安全强度字节数):</b></div>
      <TextArea
        style={{ margin: "5px 0 5px 0" }}
        value={ key }
        onChange={ (e) => { const v = e.target.value; setKey(v); recompute(v.trim() !== ''); } }
        placeholder={ fmt === 'hex'
          ? '密钥十六进制, 如 404142434445464748494a4b4c4d4e4f505152535455565758595a5b5c5d5e5f'
          : '密钥 (任意文本)' }
        autoSize={{ minRows: 2, maxRows: 3 }}
      />

      <div style={{ marginTop: 8 }}><b>消息 X:</b></div>
      <TextArea
        style={{ margin: "5px 0 5px 0" }}
        value={ data }
        onChange={ (e) => { const v = e.target.value; setData(v); recompute(key.trim() !== ''); } }
        placeholder={ fmt === 'hex' ? '消息十六进制 (可空), 如 00010203' : '消息内容 (可空)' }
        autoSize={{ minRows: 3, maxRows: 6 }}
      />

      <div style={{ marginTop: 8 }}><b>自定义字符串 S (Customization, 可空):</b></div>
      <Input
        style={{ margin: "5px 0 5px 0" }}
        value={ custom }
        onChange={ (e) => { const v = e.target.value; setCustom(v); recompute(key.trim() !== ''); } }
        placeholder='如 My Tagged Application (UTF-8 文本)'
      />

      <Row style={{ marginTop: 5 }}><Space>
        <Button
          style={{ backgroundColor: "#007bff", color: "#fff" }}
          onClick={ () => { if (key.trim() === '') { notice.warning('密钥不能为空'); return; } calc(false); } }
        >计算 KMAC</Button>
      ...
      <TextArea
        ...
        style={{ margin: "5px 0 5px 0" }}
        value={ result }
        placeholder={ `KMAC 输出 (${outLen} 字节, ${outLen * 2} 个十六进制字符)` }
