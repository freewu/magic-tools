...
const FileDiff = () => {
  ...
  const readFile = async (side: Side, file: File) => {
    try {
      ...
      notice.success(`已载入 ${side === 'a' ? '左' : '右'}侧文件: ${file.name} (${text.split('\n').length} 行)`);
    } catch (err) {
      notice.error('读取文件失败: ' + (err as Error).message);
    }
  };
    ...
    >
      { state ? (
        <div className="diff-zone-info">
          ...
          <span className="diff-filemeta">{ state.text.split('\n').length } 行 · { (state.text.length / 1024).toFixed(1) } KB</span>
        </div>
      ) : (
      ...
      { active && (
        <div className="diff-zone-side-tag">{ side === 'a' ? '左文件 (被比较基准)' : '右文件 (待比较)' }</div>
      ) }
    </div>
  );
  ...
  const status = diff ? (
    <Space size="large" style={ { marginBottom: 8 } }>
      ...
      <Text style={ { fontSize: 12, color: '#cf1322' } }>{ diff.delCount } 行仅左侧有</Text>
      <Text style={ { fontSize: 12, color: '#389e0d' } }>{ diff.insCount } 行仅右侧有</Text>
      { diff.truncated && <Text type="warning" style={ { fontSize: 12 } }>文本超长已截断比较前 4000 行</Text> }
    </Space>
  ) : null;
  ...
  return (
    <div>
      ...
      <Space wrap style={ { marginBottom: 10 } }>
        ...
        >选择右侧文件</Button>
        <Button icon={ <SwapOutlined /> } onClick={ swap } disabled={ !fileA && !fileB }>交换左右</Button>
        <Button icon={ <ClearOutlined /> } onClick={ clearAll } disabled={ !fileA && !fileB }>清除</Button>
      ...
      { rows.length === 0 ? (
        <Empty
          style={ { marginTop: 60 } }
          description="在左右区域各载入一个文件 (点击选择 / 拖拽), 自动按行 diff, 左侧红色 = 删除行, 右侧绿色 = 新增行"
        />
      ) : (
        <div className="diff-table-wrap">
          <table className="diff-table">
            <thead>
              <tr>
                <th className="diff-th diff-th-a">← { fileA?.name ?? '左侧文件' } (删除行红色)</th>
                <th className="diff-th diff-th-b">{ fileB?.name ?? '右侧文件' } (新增行绿色) →</th>
              </tr>
            </thead>
            <tbody>
          ...
          { rows.length > RENDER_LIMIT && (
            <div className="diff-more">共 { rows.length } 个 diff 行, 仅渲染前 { RENDER_LIMIT } 行</div>
          ) }
        </div>
      ) }
    </div>
