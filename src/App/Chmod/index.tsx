...
const Chmod = () => {
  ...
  const doParse = () => {
    try {
      ...
      notice.success(tt('已解读: {a} / {b}', { a: modeDigits(r.perm), b: lsSymbol(r.perm) }));
    } catch(err) {
      notice.error(tt("解读失败: {m}", { m: (err as Error).message }));
    }
  };
    ...
    <div>
      ...
      <Space wrap style={ { margin: "5px 0" } }>
        <Input
          ...
          allowClear
          value={ parseText }
          placeholder={t("输入 755 / 4755 / -rwxr-xr-x / chmod 命令")}
        ...
        <Button
          ...
          icon={<SearchOutlined />}
        >{t('解读')}</Button>
        <span style={ { color: "#888" } }>{t('解读结果会同步到下方勾选区')}</span>
      </Space>

      <Divider plain style={{ margin: "8px 0" }}>{t('配置权限 (勾选后实时生成命令)')}</Divider>

      <div style={ { margin: "4px 0 8px" } }>
        <Space wrap size="large">
          {
            targets.map((tg) => (
              <span key={ tg.key } style={ { display: "inline-block", border: "1px solid #ddd", padding: "6px 10px", borderRadius: 6 } }>
                <b style={ { marginRight: 8 } }>{ t(tg.title) }</b>
                {
                  PERM_META.map((m) => (
                    <Checkbox
                      key={ m.ch }
                      checked={ (({ user, group, other })[tg.key as 'user' | 'group' | 'other'] & m.bit) !== 0 }
                      onChange={ (e) => toggleBit(tg.key as 'user' | 'group' | 'other', m.bit, e.target.checked) }
                    >{ t(m.label) } { m.ch }</Checkbox>
                  ))
                }
              </span>
            ))
          ...
          <span style={ { display: "inline-block", border: "1px solid #ddd", padding: "6px 10px", borderRadius: 6 } }>
            <b style={ { marginRight: 8 } }>{t('特殊位')}</b>
            <Checkbox checked={ (special & 4) !== 0 } onChange={ (e) => toggleSpecial(4, e.target.checked) }>{t('setuid (4)')}</Checkbox>
            <Checkbox checked={ (special & 2) !== 0 } onChange={ (e) => toggleSpecial(2, e.target.checked) }>{t('setgid (2)')}</Checkbox>
            <Checkbox checked={ (special & 1) !== 0 } onChange={ (e) => toggleSpecial(1, e.target.checked) }>{t('sticky (1)')}</Checkbox>
          </span>
        </Space>
      </div>
      ...
      <Table
        ...
        dataSource={ meanings }
        columns={ [
          { title: t('对象'), dataIndex: 'target', render: (v: string) => t(v) },
          { title: t('数值'), dataIndex: 'bits', width: 60 },
          { title: t('符号'), dataIndex: 'chars', width: 80 },
          { title: t('权限含义'), render: (_ :unknown, r :ChmodMeaning) =>
              [r.read ? t('读') : '', r.write ? t('写') : '', r.exec ? t('执行') : ''].filter(Boolean).join(' / ') || t('无权限'),
            width: 160 },
          { title: t('特殊位'), dataIndex: 'special', width: 160 },
        ] }
      />
      ...
      <Divider>{t(' chmod 权限说明 ')}</Divider>
      <p style={{ margin: "4px 0 8px", fontSize: 13 }}>
        {t('Linux 权限分三组：属主 (u)、属组 (g)、其它 (o)，每组含读 (r=4)、写 (w=2)、执行 (x=1) 三个位，数值求和即为 0-7 的数字权限。数字串从左到右依次为 u/g/o，如')}{' '}<code>chmod 755</code>{' '}{t('= u:rwx, g:r-x, o:r-x。')}
      </p>
      <p style={{ margin: "4px 0 8px", fontSize: 13 }}>
        {t('首位为特殊位：4 = setuid（执行时以属主身份运行，如 passwd）、2 = setgid（目录内新建文件继承属组）、1 = sticky（仅属主可删除，如 /tmp）。ls 显示时 setuid/setgid 使 x 变为小写 s（无执行则为大写 S），sticky 使其它执行位变为 t/T。')}
      </p>
      <Table
        size="small"
        rowKey="num"
