...
import unicodeLang from "./lang";

const Unicode = () => {

  const { locale } = useLocale();
  const t = (key: string, fallback: string) => tr(unicodeLang, locale, key, fallback);
  ...
  return (
    <div>
      ...
      <TextArea
        ...
        value= { encodeValue }
        placeholder={ t('encodePh', '输入需要进行 Unicode 编码的内容  或 拖拽文件到框内打开') }
      ...
      <TextArea
        ...
        title={ t('copyTitle', '双击复制内容到粘贴板') }
        value= { decodeValue }
        placeholder={ t('decodePh', '输入需要进行 Unicode 解码的内容  或 拖拽文件到框内打开') }
        autoSize={{ minRows: 5, maxRows: 5 }}
        onDragOver={ (e) => { e.preventDefault(); } } // 必须加上，否则无法触发下面的方法
        onDrop={ (e) => { e.preventDefault(); openFile(e.dataTransfer.files, setDecodeValue ); } }
      />
      
      <Divider>{ t('divider', 'Unicode 编码说明') }</Divider>
