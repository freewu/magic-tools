...
const rows: Record<string, [string, string]> = {
  ...
  '生成数量与默认格式 / 默认表名可在「设置 → 生成器 → 数据生成」中调整; 数量上限 1000 条。': ['產生數量與預設格式 / 預設表名可在「設定 → 產生器 → 資料產生」中調整; 數量上限 1000 筆。', 'The record count, default format and default table name can be changed under Settings → Generators → Mock Data; the limit is 1000 records.'],
