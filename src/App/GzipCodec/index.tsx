...
import gzipLang from "./lang";

const GzipCodec = () => {
  const { locale } = useLocale();
  const t = (key: string, fallback: string) => tr(gzipLang, locale, key, fallback);
  const tpl = (key: string, vars: Record<string, string | number>, fallback: string) => trTpl(gzipLang, locale, key, vars, fallback);
  ...
  const doGzip = async () => {
    if (!plain.trim()) { message.warning(t('noPlain', '请先输入要压缩的内容')); return; }
    setBusy('gzip');
    try {
      ...
      message.error(e instanceof Error ? e.message : t('compressFail', '压缩失败'));
    } finally {
      setBusy(null);
    }
  };
  ...
  const doGunzip = async () => {
    ...
    if (!input) { message.warning(t('noInput', '请先输入要解压的 Gzip 串 (Base64 或 Hex)')); return; }
    setBusy('gunzip');
    try {
      ...
      message.error(e instanceof Error ? e.message : t('gunzipFail', '解压失败'));
    } finally {
      setBusy(null);
    }
  };
  ...
  const textareaCopy = (e: React.MouseEvent<HTMLTextAreaElement>) => {
    ...
    if (txt) {
      ...
      message.success(t('copied', '已复制到剪贴板'));
    }
  };

  const btnStyle = { color: '#fff' } as const;
  const statText = stat
    ? tpl('statText', { src: stat.src, dst: stat.dst, pct: stat.src > 0 ? Math.round(stat.dst / stat.src * 100) : 0 }, '')
    : '';
  ...
  return (
    <div style={ { maxWidth: 900 } }>
      <Input.TextArea
        ...
        title={ t('copyTitle', '双击复制内容到剪贴板') }
        placeholder={ t('plainPh', '输入要 Gzip 压缩的文本, 或拖拽文件到框内以文件内容为输入') }
      ...
      <div style={ { display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, margin: '8px 0' } }>
        <Button type="primary" icon={ <ArrowDownOutlined /> } loading={ busy === 'gzip' }
          disabled={ busy !== null } onClick={ doGzip }>{ t('gzip', 'Gzip 压缩') }</Button>
        <Button style={ { ...btnStyle, background: '#28a745', borderColor: '#28a745' } }
          icon={ <ArrowUpOutlined /> } loading={ busy === 'gunzip' }
          disabled={ busy !== null } onClick={ doGunzip }>{ t('gunzip', 'Gzip 解压') }</Button>
        <Radio.Group
          value={ fmt }
        ...
        <Button icon={ <ClearOutlined /> }
          onClick={ () => { setPlain(''); setCodec(''); lastBytesRef.current = null; setStat(null); } }>{ t('clear', '清除') }</Button>
      </div>
      ...
      <Input.TextArea
        ...
        title={ t('copyTitle', '双击复制内容到剪贴板') }
        placeholder={ fmt === 'hex'
          ? t('resultHexPh', 'Gzip 压缩结果 (Hex, 可双击复制), 或粘贴待解压的 Gzip 串 (Base64/Hex 均可)')
          : t('resultB64Ph', 'Gzip 压缩结果 (Base64, 可双击复制), 或粘贴待解压的 Gzip 串 (Base64/Hex 均可)') }
        autoSize={ { minRows: 5, maxRows: 8 } }
        onDragOver={ (e) => e.preventDefault() }
      ...
      <Space style={ { marginTop: 4 } } size={ 16 }>
        <Typography.Text type="secondary" style={ { fontSize: 12 } }>
          { statText || t('statHint', '压缩率按输入输出字节计算; 点击下方说明了解格式差异') }
        </Typography.Text>
      </Space>
