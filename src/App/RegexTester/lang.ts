...
const uilangRows: Record<string, [string, string]> = {
  ...
  '请先在上方输入正则表达式, 这里会生成对应语言的调用代码': ['請先在上方輸入正則表達式, 這裡會產生對應語言的呼叫程式碼', 'Enter a regex above to generate usage code for the selected language'],
  '示例文本取自上方的输入内容, 最多前 {n} 行': ['範例文字取自上方輸入內容, 最多前 {n} 行', 'Sample text comes from the input above (first {n} lines at most)'],
  '可选的示例文本取自上方的输入内容': ['可選的範例文字取自上方輸入內容', 'Sample text comes from the input above'],
  '字节': ['位元組', 'bytes'],
  '次': ['次', 'times'],
  '位': ['位', 'bits'],
  'r:': ['r:', 'r:'],
