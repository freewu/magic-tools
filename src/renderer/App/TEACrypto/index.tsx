...
import { getDefaultCode, getDefaultPassphrase, getDefaultRound } from "./lib";
...
const RabbitCrypto = () => {
  ...
  const [ valueStatus, setValueStatus ] = useState( '' as InputStatus); // 加密内容提醒
  ...
  const isCanDo = (value :string) :boolean => {
    ...
    // 内容需要满足按 8字节分组
    if(encodeValue.length % 8 !== 0) return false;
